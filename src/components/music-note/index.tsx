...
const MusicNote = forwardRef((props: any, ref: any) => {
  const iconsRef = useRef() as any;
  // 容器总有3个音符，也就是同时只能有3个音符下落
  const ICON_NUMBER = 3;
  ...
  useEffect(() => {
    ...
    // 类数组转换成数组，当然也可以用[...xxx]解构语法或者Array.from()
    let domArray = [].slice.call(iconsRef.current.children);
    domArray.forEach((item: any) => {
      item.running = false;
      item.addEventListener('transitionend', function (this: any) {
